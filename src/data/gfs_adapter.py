...
class GFSAdapter(BaseDataAdapter):
    """
    Adapter for ECMWF GFS Forecasts via Open-Meteo API.
    """
    def __init__(self, cache_dir: Path, **kwargs):
        super().__init__(source_name="GFS", cache_dir=cache_dir, **kwargs)
    ...
    def fetch_forecast(
        ...
        """
        Fetch GFS forecast data from Open-Meteo.
        """
        identifier = f"GFS_REAL_{init_time.isoformat()}_{variables}_{lat_range}_{lon_range}_{lead_times_hours}"
        cached = self._load_from_cache(identifier)
        if cached is not None:
            return cached

        self.logger.info(f"Fetching real GFS forecast for {init_time}")
        ...
        ds = xr.Dataset(
            ...
            attrs={
                'description': 'Real GFS Forecast from Open-Meteo',
