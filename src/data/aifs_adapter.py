...
class AIFSAdapter(BaseDataAdapter):
    """
    Adapter for ECMWF AIFS Forecasts via Open-Meteo API.
    """
    def __init__(self, cache_dir: Path, **kwargs):
        super().__init__(source_name="AIFS", cache_dir=cache_dir, **kwargs)
    ...
    def fetch_forecast(
        ...
        """
        Fetch AIFS forecast data from Open-Meteo.
        """
        identifier = f"AIFS_REAL_{init_time.isoformat()}_{variables}_{lat_range}_{lon_range}_{lead_times_hours}"
        cached = self._load_from_cache(identifier)
        if cached is not None:
            return cached

        self.logger.info(f"Fetching real AIFS forecast for {init_time}")
        ...
        ds = xr.Dataset(
            ...
            attrs={
                'description': 'Real AIFS Forecast from Open-Meteo',
