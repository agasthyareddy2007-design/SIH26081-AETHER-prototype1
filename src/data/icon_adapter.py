...
class ICONAdapter(BaseDataAdapter):
    """
    Adapter for ECMWF ICON Forecasts via Open-Meteo API.
    """
    def __init__(self, cache_dir: Path, **kwargs):
        super().__init__(source_name="ICON", cache_dir=cache_dir, **kwargs)
    ...
    def fetch_forecast(
        ...
        """
        Fetch ICON forecast data from Open-Meteo.
        """
        identifier = f"ICON_REAL_{init_time.isoformat()}_{variables}_{lat_range}_{lon_range}_{lead_times_hours}"
        cached = self._load_from_cache(identifier)
        if cached is not None:
            return cached

        self.logger.info(f"Fetching real ICON forecast for {init_time}")
        ...
        ds = xr.Dataset(
            ...
            attrs={
                'description': 'Real ICON Forecast from Open-Meteo',
